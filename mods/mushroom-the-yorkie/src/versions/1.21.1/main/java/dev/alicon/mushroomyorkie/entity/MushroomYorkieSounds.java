...
final class MushroomYorkieSounds {
	...
	static void playTreatTrick(MushroomYorkieEntity yorkie) {
		...
		switch (trick) {
			case 0 -> yorkie.playSound(SoundEvents.WOLF_PANT, 0.45F, 1.45F);
			case 1 -> yorkie.playSound(SoundEvents.WOLF_PANT, 0.5F, 1.45F);
			case 2 -> yorkie.setDeltaMovement(yorkie.getDeltaMovement().add(0.0D, 0.28D, 0.0D));
			default -> yorkie.playSound(SoundEvents.WOLF_GROWL, 0.45F, 1.55F);
		}
	...
	static void playSleepWake(MushroomYorkieEntity yorkie) {
		yorkie.playSound(SoundEvents.WOLF_PANT, 0.35F, 1.45F);
	}

	static void bark(MushroomYorkieEntity yorkie) {
		yorkie.playSound(SoundEvents.WOLF_AMBIENT, 0.5F, 1.45F);
	}

	static void whine(MushroomYorkieEntity yorkie) {
		yorkie.playSound(SoundEvents.WOLF_WHINE, 0.55F, 1.45F);
	}

	static void playScoldedWhine(MushroomYorkieEntity yorkie) {
		yorkie.playSound(SoundEvents.WOLF_WHINE, 0.7F, 1.45F);
	}

	static void playHostileGrowl(MushroomYorkieEntity yorkie) {
		yorkie.playSound(SoundEvents.WOLF_GROWL, 0.45F, 1.6F);
	}

	static void playFoodSound(MushroomYorkieEntity yorkie) {
		yorkie.playSound(SoundEvents.GENERIC_EAT, 0.45F, 1.45F);
	}
